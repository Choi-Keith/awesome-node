  ...
  <div class="table">
    ...
    <div class="container">
      <div class="handle-box">
        <el-input v-model="searchName" placeholder="搜索预约姓名" class="handle-input mr10"></el-input>
      ...
      <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="type" label="装修类型">
        </el-table-column>
        ...
        <el-table-column prop="message" label="操作">
          <template slot-scope="scope">
            <div>
              <el-button type="primary" @click="showUpdateStatus(scope.row.id, 1, '已联系')" v-if="scope.row.status==0">已联系</el-button>
            </div>
            <div style="margin-top: 10px;">
              <el-button type="success" @click="showUpdateStatus(scope.row.id, 2, '已确定')" v-if="scope.row.status==0 || scope.row.status==1">已确定</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 修改状态提示框 -->
    <el-dialog title="修改预约状态提示" :visible.sync="updateStatusVisible" width="400px" center>
      <div class="del-dialog-cnt">你确定要修改预约状态为 {{updateStatusName}} 吗？</div>
